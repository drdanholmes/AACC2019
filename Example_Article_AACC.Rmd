...
If you need to make reference to a figure, for example to figure \@ref(fig:hist), you do it by the *name* and not by the *number* so that if the sigure order changes, the numbering will change automatically.
...
knitr::include_graphics("Images/darth")
```

# Insertion of Tables

Tables need to be outputed through a function in order that they look nice.  Ultimately, there is total control of the table appearance using \LaTeX, but this is outside our scope. Simple tables can be inserted with the `kable` package.
...
Of course, you can take the output of a statistical analsis and put it into a table as shown in table \@ref(tab:bigtable):
...
Here is an article that Shannon Haymond wrote [@haymond2005laboratory] and here is one that Dan Holmes wrote that has fewer citations and is not as important [@rehan2015laboratory]. If we wanted to cite them both together we can [@haymond2005laboratory; @rehan2015laboratory].

## CSL files and Reference Abbreviation

Abbreviation of reference names is not mandatory if you are submitting to an Elsevier journal but if you are submitted as a WORD document to a regular journal, you will need to abbreviate your references as per the journal's requirements. I have prepared a .json file for you from the *entire* web of science list of journals. It is stored in the Extras folder. This will permit journal abbreviation. To choose the right reference formating you need to choose the right .csl file from the [github csl repository](https://github.com/citation-style-language/styles). You can download the sourcecode for this article [here](https://github.com/drdanholmes/AACC2019).
...
+ [Here is a helpful artilce](https://libscie.github.io/rmarkdown-workshop/handout.html).
